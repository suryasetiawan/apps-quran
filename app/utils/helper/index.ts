...
export const validateTime = (time, date) => {
  ...
  let month = date ? date.getMonth() : new Date().getMonth()
  let day = date ? date.getDate() : new Date().getDate()
  if (time && time.length === 5 && time[2] === ':') {
    if (isNaN(time[0] + time[1]) && isNaN(time[3] + time[4])) {
      return false
    } else {
      if (Number(time[0] + time[1]) <= 24 && time[3] + time[4] <= 60) {
        finalVal = new Date(
          year,
          month,
          day,
          Number(time[0] + time[1]),
          Number(time[3] + time[4]),
        )
        return finalVal
      } else {
        return false
